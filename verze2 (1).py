...
if myDlg.OK:
    ...
    datalog.write("\n" + ok_data)
    datalog.close()
    datalog=open('C:/Users/stazisti/Desktop/indif/all_2.txt', "at")
    datalog.write("\n" + ok_data)
    datalog.close()
    datalog=open('C:/Users/stazisti/Desktop/indif/all_ver2.txt', "at")
    datalog.write("\n" + ok_data)
    datalog.close()
...
wait(3)
intro.text = u"Experiment bude mít dvě části: první bude s vizuálními, druhá s akustickými stimuly.\n\nPro pokračování stiskněte mezerník nebo tlačítko 2."
intro.draw()
my_win.flip()
...
def practise_vis():
    ...
    my_win.flip()
    wait(3)
    practise.text = u"V této části budete posuzovat délku trvání vizuálního stimulu na obrazovce.\n\n<mezerník/2>"
    practise.draw()
    my_win.flip()
    waitKeys(keyList = ["space","b"])
    ...
    my_win.flip()
    waitKeys(keyList = ["space","b"])
    practise.text = "Stisknutím mezerníku nebo tlačítka 2 IHNED zobrazíte první zkušební stimul.\n\n<mezerník/2>"
    practise.draw()
    my_win.flip()
    waitKeys(keyList = ["space","b"])
...
def block_vis():
    ...
    for i in range (9):
        ...
        datalog.close()
        datalog=open('C:/Users/stazisti/Desktop/indif/all.txt', "at")
        datalog.write(","+str(t_exp_begin)+","+str(t_repro_begin)+","+str(choice_exp)+","+str(key.duration)+","+str(key.name)+","+str(key.rt)+","+str(t_trial_end))
        datalog.close()
        datalog=open('C:/Users/stazisti/Desktop/indif/all_2.txt', "at")
        datalog.write(","+str(t_exp_begin)+","+str(t_repro_begin)+","+str(choice_exp)+","+str(key.duration)+","+str(key.name)+","+str(key.rt)+","+str(t_trial_end))
        datalog.close()
...
my_win.flip()
wait(3)
intro.text = u"V této části budete posuzovat délku trvání zvukových stimulů.\n\n<mezerník/2>"
intro.draw()
my_win.flip()
t_acu_s=my_time.getTime()
waitKeys(keyList = ["space","b"])
intro.text = "Prosíme, reprodukujte celou dobu trvání stimulu. \n\n<mezerník/2>"
intro.draw()
my_win.flip()
t_whole=my_time.getTime()
waitKeys(keyList = ["space","b"])
...
my_win.flip()
wait(3)
practise.text = "Stisknutím mezerníku nebo tlačítka 2 nyní IHNED spustíte první zkušební nahrávku.\n\n<mezerník/2>"
practise.draw()
my_win.flip()
waitKeys(keyList = ["space","b"])
datalog=open('C:/Users/stazisti/Desktop/indif/mylogfile.txt', "at")
datalog.write(u"intro_acu:"+ "\n" + u"t_acu_s:\t" + str(t_acu_s) + u"\nt_whole:\t" + str(t_whole) + u"\nt_not_count:\t" + str(t_not_count))
datalog.close()
...
def block_acu():
    ...
    for i in range (9):
        ...
        while True:
            ...
            experiment.draw()
            my_win.flip()
            waitKeys(keyList = ["right","b"])
            t_arr_down=my_time.getTime()
            keys=kb.waitKeys(keyList = ["space","c","b","right","q"], waitRelease=True)
            if 'q' in keys:
                core.quit()
            for key in keys:
